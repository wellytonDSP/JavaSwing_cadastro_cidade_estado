 ...
 * @author Admin
 */
public class NumberFormatedField extends JTextField {

    public PlainDocument getDocumento() {
        try {
            return (PlainDocument) getDocument();
        } catch (Exception e) {
            return null;
        }
    ...
    public NumberFormatedField(int casasDecimais) {
        this(new DecimalFormat((casasDecimais == 0 ? "#,##0" : "#,##0.") + makeZeros(casasDecimais)));
    }

    public NumberFormatedField() {
        this(MONETARY_FORMAT);
    }

    public NumberFormatedField(NumberFormat format) {// define o formato do
        // número
        ...
        setDocument(new PlainDocument() {
            ...
            public void insertString(int offs, String str, AttributeSet a) throws BadLocationException {
                String text = new StringBuilder(NumberFormatedField.this.getText().replaceAll("[^0-9]", "")).append(str.replaceAll("[^0-9]", "")).toString();
